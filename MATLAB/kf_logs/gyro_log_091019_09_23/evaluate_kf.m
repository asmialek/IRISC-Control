...
%% nu_next.log
opts = detectImportOptions('kf/nu_next.log');
opts = setvartype(opts, 2, 'double');
opts = setvaropts(opts, 2, 'DecimalSeparator', '.');

[~,nu] = readvars('kf/nu_next.log', opts);

%% p_upd.log
opts = detectImportOptions('kf/p_upd.log');
opts = setvartype(opts, 2:5, 'double');
opts = setvaropts(opts, 2:5, 'DecimalSeparator', '.');

[~,P11, P12, P21, P22] = readvars('kf/p_upd.log', opts);

%% s_next.log
opts = detectImportOptions('kf/s_next.log');
opts = setvartype(opts, 2, 'double');
opts = setvaropts(opts, 2, 'DecimalSeparator', '.');

[~,S] = readvars('kf/s_next.log', opts);

%% alt_upd.log
opts = detectImportOptions('kf/alt_upd.log');
opts = setvartype(opts, 2:3, 'double');
opts = setvaropts(opts, 2:3, 'DecimalSeparator', '.');

[~,alt_est, beta_alt_est] = readvars('kf/alt_upd.log', opts);

%% x_upd.log
opts = detectImportOptions('kf/x_upd.log');
opts = setvartype(opts, 2:3, 'double');
opts = setvaropts(opts, 2:3, 'DecimalSeparator', '.');

[~,alt_est, beta_alt_est] = readvars('kf/x_upd.log', opts);
...
alt_est = alt_est(1:N,1) - 1;
beta_alt_est = beta_alt_est(1:N,1);
P11 = P11(1:N,1);
P12 = P12(1:N,1);
P21 = P21(1:N,1);
P22 = P22(1:N,1);
